...
export function ResultsPage(props: sharedProps) {
    ...
    const [idFeatureMap] = useState<Map<string, featuresResponse>>(new Map<string, featuresResponse>())
    const [currId, setCurrId] = useState<string>("");
    const [currFeatures, setCurrFeatures] = useState<featuresResponse | undefined>()
        ...
        if (feats != undefined) {
          for (let i = 0; i < genPlaylist.data.length; i ++) {
            if (i == 0) {
              ...
              setCurrSong(props.returnedTracks.filter(x => x.id === feats[i].id)[0])
            }
          ...
          <main className="container-fluid">
            ...
            <div style = {{display: "flex"}}>
              ...
              <div className="recommended-track-container">
                ...
                {props.returnedTracks.map((track:trackResponse) => (<RecommendedTrackCard currFeature = {currFeatures} setCurrFeatures = {setCurrFeatures}
                currId = {currId} setCurrId = {setCurrId} currSong = {currSong} setCurrSong = {setCurrSong} idFeatureMap = {idFeatureMap}resultInfo={track} key={track.id}/>))}
              </div>
