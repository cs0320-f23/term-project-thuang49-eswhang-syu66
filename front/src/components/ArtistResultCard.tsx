...
export function ArtistResultCard(props: prop) {
    ...
    function addArtist() {
        ...
        if (!(props.selectedSeeds.map(x => x[0]).includes(props.resultInfo.id))) {
            ...
            props.setSelectedSeeds([... props.selectedSeeds, [props.resultInfo.id, "artists", props.resultInfo.name, returnImages()]])
        } else {
    ...
    return (
        <div className = {"artist-result-card"} id = {"artists"+props.resultInfo.id} onClick = {addArtist}>
